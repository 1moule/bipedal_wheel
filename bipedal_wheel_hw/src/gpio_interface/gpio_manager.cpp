...
{
void GpioManager::setGpioDirection(hardware_interface::gpio::GpioData gpioData)
{
  ...
  if (fd == -1) {
    ...
    if (gpioData.type == hardware_interface::gpio::OUTPUT) {
      if (write(fd, "out", 3) != 3) {
...
void GpioManager::readGpio(std::vector<hardware_interface::gpio::GpioData> & gpio_state_values)
{
  for (auto iter = gpio_state_values.begin(); iter != gpio_state_values.end(); iter++) {
    if (iter->type == hardware_interface::gpio::INPUT) {
...
void GpioManager::writeGpio(
  const std::vector<hardware_interface::gpio::GpioData> & gpio_command_values)
{
  char buffer[1] = {'1'};
  for (auto iter : gpio_command_values) {
    ...
    if (fd == -1) {
      ...
      if (*iter.value) {
        ...
        if (ref == -1) ROS_ERROR("[GPIO]Failed to write to gpio%d.", iter.pin);
      } else {
        buffer[0] = '0';
        int ref = write(fd, buffer, 1);
        if (ref == -1) ROS_ERROR("[GPIO]Failed to write to gpio%d.", iter.pin);
