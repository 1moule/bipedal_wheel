  ...
  : name_(std::move(name)), type_(type), value_(value)
  {
    if (!value)
      throw hardware_interface::HardwareInterfaceException(
        "Cannot create handle '" + name + "'. value pointer is null.");
  }
  std::string getName() const { return name_; }
  GpioType getType() const { return type_; }
  bool getValue() const
  {
    assert(value_);
    return *value_;
...
public:
  ...
  : name_(std::move(name)), type_(type), cmd_(cmd)
  {
    if (!cmd)
      throw hardware_interface::HardwareInterfaceException(
        "Cannot create handle '" + name + "'. command pointer is null.");
  }
  std::string getName() const { return name_; }
...
class GpioStateInterface : public hardware_interface::HardwareResourceManager<
                             GpioStateHandle, hardware_interface::DontClaimResources>
{
};

class GpioCommandInterface : public hardware_interface::HardwareResourceManager<
