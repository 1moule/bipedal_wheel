...
namespace bipedal_wheel_localization
{
Scan2MapLocation::Scan2MapLocation() : private_node_("~"), tf_listener_(tfBuffer_)
{
  ...
  laser_scan_subscriber_ = node_handle_.subscribe(
    "/scan", 1, &Scan2MapLocation::ScanCallback, this, ros::TransportHints().tcpNoDelay());

  map_subscriber_ = node_handle_.subscribe("/map", 1, &Scan2MapLocation::MapCallback, this);

  odom_subscriber_ = node_handle_.subscribe(
    "/odom", 20, &Scan2MapLocation::OdomCallback, this, ros::TransportHints().tcpNoDelay());

  map_pointcloud_publisher_ =
    node_handle_.advertise<sensor_msgs::PointCloud2>("map_pointcloud", 10);

  scan_pointcloud_publisher_ =
    node_handle_.advertise<sensor_msgs::PointCloud2>("scan_pointcloud", 10);

  removal_pointcloud_publisher_ =
    node_handle_.advertise<sensor_msgs::PointCloud2>("removal_pointcloud", 10);
  ...
  location_publisher_ =
    node_handle_.advertise<geometry_msgs::PoseWithCovarianceStamped>("location_match", 1, this);

  relocate_tranform_visuial_publisher_ =
    node_handle_.advertise<geometry_msgs::PoseWithCovarianceStamped>(
      "relocate_visuial_pose", 1, this);

  rotate_robotpose_publisher_ =
    node_handle_.advertise<geometry_msgs::PoseWithCovarianceStamped>("rotate_robot_pose", 1, this);

  relocate_initialpose_publisher_ =
    node_handle_.advertise<geometry_msgs::PoseWithCovarianceStamped>("initialpose", 1, this);

  location_info_publisher_ =
    node_handle_.advertise<bipedal_wheel_msgs::LocationInfo>("location_info", 1, this);

  relocalization_srv_ =
    node_handle_.advertiseService("relocalization", &Scan2MapLocation::RelocalizeCallback, this);
  ...
  ROS_INFO_STREAM("Initial successfully");
}
...
void Scan2MapLocation::InitParams()
{
  ...
  private_node_.param<double>(
    "ANGLE_SPEED_THRESHOLD", ANGLE_SPEED_THRESHOLD_, 7);            //角速度阈值，大于此值不发布结果
  ...
  private_node_.param<double>(
    "SCORE_THRESHOLD_MAX", SCORE_THRESHOLD_MAX_,
    0.1);  //达到最大迭代次数或者到达差分阈值后后，代价仍高于此值，认为无法收敛,自适应使用
  private_node_.param<double>(
    "Point_Quantity_THRESHOLD", Point_Quantity_THRESHOLD_, 200);  //点云数阈值,低于此值不匹配
  private_node_.param<double>(
    "Maximum_Iterations", Maximum_Iterations_, 100);  //ICP中的最大迭代次数
  ...
  private_node_.param<bool>(
    "Use_TfTree_Always", Use_TfTree_Always, true);  //是否总是使用tf树读取变换
  ...
  private_node_.param<double>(
    "ObstacleRemoval_Distance_Max", ObstacleRemoval_Distance_Max, 2);  //最大距离

  // 定位禁区、
  private_node_.param<std::vector<double>>(
    "location_restricted_zone", location_restricted_zone_, {});
  // std::cout << location_restricted_zone_[0] << std::endl;

  // 重定位
  private_node_.param<double>(
    "Relocation_Weight_Score", Relocation_Weight_Score_, 0.5);  //重定位分数系数
  private_node_.param<double>(
    "Relocation_Weight_Distance", Relocation_Weight_Distance_, 0.5);  //重定位距离系数
  private_node_.param<double>(
    "Relocation_Weight_Yaw", Relocation_Weight_Yaw_, 0.5);  //重定位yaw角系数
  private_node_.param<double>(
    "Relocation_Maximum_Iterations", Relocation_Maximum_Iterations_, 80);  //重定位最大迭代次数
  private_node_.param<double>(
    "Relocation_Score_Threshold_Max", Relocation_Score_Threshold_Max_, 0.15);

  private_node_.param<int>(
    "Loss_Num_Threshold", Loss_Num_Threshold_, -1);  //丢失阈值，丢失次数大于此值进入重定位
}
 ...
 * 重定位服务回调函数
*/
bool Scan2MapLocation::RelocalizeCallback(
  bipedal_wheel_msgs::Relocate::Request & req, bipedal_wheel_msgs::Relocate::Response & res)
{
  ros::Rate rate(10);
...
void Scan2MapLocation::ScanCallback(const sensor_msgs::LaserScan::ConstPtr & scan_msg)
{
  ...
  location_match.pose.covariance = {Variance_X, 0, 0, 0, 0, 0, 0, Variance_Y,  0, 0, 0, 0, 0, 0,
                                    1e-9,       0, 0, 0, 0, 0, 0, 1e-9,        0, 0, 0, 0, 0, 0,
                                    1e-9,       0, 0, 0, 0, 0, 0, Variance_Yaw};
  location_publisher_.publish(location_match);  //发送匹配结果

  //计算函数用时并发布
  scan_end_time_ = std::chrono::steady_clock::now();
  scan_time_used_ =
    std::chrono::duration_cast<std::chrono::duration<double>>(scan_end_time_ - scan_start_time_);
  if (if_debug_) {
    std::cout << "ScanCallBack整体函数处理用时: " << scan_time_used_.count() << " 秒。"
              << std::endl;
  }

  scan_time_used_ =
    std::chrono::duration_cast<std::chrono::duration<double>>(scan_end_time_ - scan_last_end_time_);
  if (if_debug_) {
    std::cout << "ScanCallback函数频率为: " << 1.0 / scan_time_used_.count() << " HZ。"
              << std::endl;
  }
 ...
 * 使用ICP进行帧间位姿的计算
 */
bool Scan2MapLocation::ScanMatchWithICP(
  Eigen::Isometry3d & trans, PointCloudT::Ptr & cloud_scan_msg, PointCloudT::Ptr & cloud_map_msg)
{
  ...
  if (if_debug_) {
    std::cout << "tranDist:" << tranDist << " angleDist: " << angleDist
              ...
              << " angle speed: " << odom_queue_.back().twist.twist.angular.z << std::endl;
  }

  bipedal_wheel_msgs::LocationInfo location_info_msg;
  ...
  // 如果变换小于一定值，不发布结果，退出
  if (
    tranDist < DIST_THRESHOLD_ && angleDist < ANGLE_THRESHOLD_ ||
    cloud_scan_msg->points.size() < Point_Quantity_THRESHOLD_) {
    if (if_debug_) {
      // \033[1;33m，\033[0m 终端显示成黄色
      std::cout << "\033[1;33m" << "Distance or point_Quantity out of threshold" << "\033[0m"
                << std::endl;
      std::cout << "\033[1;33m" << "tranDist:" << tranDist << " angleDist: " << angleDist
                << " score: " << icp_.getFitnessScore()
                << " angle speed: " << odom_queue_.back().twist.twist.angular.z << "\033[0m"
                << std::endl;
    }
  ...
  //如果匹配结果不满足条件，退出
  if (
    angleDist > ANGLE_UPPER_THRESHOLD_ || icp_.getFitnessScore() > SCORE_THRESHOLD_MAX_ ||
    ...
    if (
      location_loss_num_ > Loss_Num_Threshold_ &&
      Loss_Num_Threshold_ != -1)  //丢失超过阈值进入重定位
    {
      need_relocalization = true;
    }
    if (if_debug_) {
...
*/
bool Scan2MapLocation::ReLocationWithICP(
  Eigen::Isometry3d & trans, const sensor_msgs::LaserScan::ConstPtr & scan_msg,
  PointCloudT::Ptr & cloud_map_msg, const Eigen::Isometry3d & robot_pose)
{
  ...
  if (save_pcd_) {
    ...
    std::string scan_cloud_filename =
      std::string("/home/autorobot/4ws_auto_infantry_ws/src/scan_to_map/bagfiles/scan_cloud_") +
      buffer + ".pcd";
    std::string map_cloud_filename =
      std::string("/home/autorobot/4ws_auto_infantry_ws/src/scan_to_map/bagfiles/map_cloud_") +
      buffer + ".pcd";
  ...
  for (int i = 0; i < initial_transforms.size(); i++) {
    ...
    float fitness_score = Relocation_Weight_Score_ * icp_.getFitnessScore() +
                          Relocation_Weight_Distance_ * tranDist +
                          Relocation_Weight_Yaw_ * angleDist;
    ...
    initial_transform.translation() =
      initial_transforms[i].cast<double>().matrix().block<3, 1>(0, 3);
    ...
    Eigen::Isometry3d relocate_tranform_visuial_ =
      Eigen::Isometry3d::Identity();  //重定位过程可视化坐标
  ...
  initial_transform.linear() =
    initial_transforms[best_index].cast<double>().matrix().block<3, 3>(0, 0);
  initial_transform.translation() =
    initial_transforms[best_index].cast<double>().matrix().block<3, 1>(0, 3);
  ...
  if (if_debug_) {
    ...
    std::cout << "Best transformation matrix:" << std::endl
              << Transformation_sources[best_index].matrix() << std::endl;
  }
 ...
 */
void Scan2MapLocation::PointCloudObstacleRemoval(
  PointCloudT::Ptr & cloud_map_msg, PointCloudT::Ptr & cloud_msg, double Distance_Threshold)
{
...
void Scan2MapLocation::PointCloudVoxelGridRemoval(PointCloudT::Ptr & cloud_msg, double leafSize)
{
  ...
  voxel_grid.setLeafSize(
    leafSize, leafSize, leafSize);  //注：体素不一定是正方体，体素可以是长宽高不同的长方体
 ...
 */
void Scan2MapLocation::OccupancyGridToPointCloud(
  const nav_msgs::OccupancyGrid::ConstPtr & map_msg, PointCloudT::Ptr & cloud_msg)
{
 ...
 */
void Scan2MapLocation::ScanToPointCloudOnMap(
  const sensor_msgs::LaserScan::ConstPtr & scan_msg, PointCloudT::Ptr & cloud_msg)
{
  ...
  for (unsigned int i = 0; i < scan_msg->ranges.size(); ++i) {
    ...
    if (
      (range > scan_msg->range_min) && (range < scan_msg->range_max) && (range > Scan_Range_Min) &&
  ...
  tran_time_used_ =
    std::chrono::duration_cast<std::chrono::duration<double>>(tran_end_time_ - tran_start_time_);
 ...
 * 对点云数据绕机器人坐标点进行旋转
*/
void Scan2MapLocation::rotatePointCloud(
  PointCloudT::Ptr & cloud_msg, const Eigen::Affine3f & rotation, const Eigen::Affine3f & robo_pose)
{
  //将点云转换到原点
 ...
 */
bool Scan2MapLocation::GetOdomTransform(
  Eigen::Isometry3d & trans, double start_time, double end_time)
{
  ...
  tf2::Quaternion quat_start(
    start_odom_msg_.pose.pose.orientation.x, start_odom_msg_.pose.pose.orientation.y,
    start_odom_msg_.pose.pose.orientation.z, start_odom_msg_.pose.pose.orientation.w);
  ...
  tf2::Quaternion quat_end(
    end_odom_msg_.pose.pose.orientation.x, end_odom_msg_.pose.pose.orientation.y,
    end_odom_msg_.pose.pose.orientation.z, end_odom_msg_.pose.pose.orientation.w);
...
bool Scan2MapLocation::Get2TimeTransform(Eigen::Isometry3d & trans)
{
  ...
  try {
    transformStamped =
      tfBuffer_.lookupTransform(map_frame_, base_frame_, ros::Time(0), ros::Duration(0));
  ...
  transEnd.pretranslate(Eigen::Vector3d(
 ...
 * trans 类型： Eigen::Isometry3d
 */
bool Scan2MapLocation::GetTransform(
  Eigen::Isometry3d & trans, const std::string parent_frame, const std::string child_frame,
  const ros::Time stamp)
{
  ...
  try {
    gotTransform = true;
    transformStamped =
      tfBuffer_.lookupTransform(parent_frame, child_frame, stamp, ros::Duration(1.0));
  ...
  trans.pretranslate(Eigen::Vector3d(
    transformStamped.transform.translation.x, transformStamped.transform.translation.y,
    transformStamped.transform.translation.z));
...
geometry_msgs::PoseWithCovarianceStamped Scan2MapLocation::Isometry3d_to_PoseWithCovarianceStamped(
...
bool Scan2MapLocation::is_coordinate_in_range(
  const std::vector<double> & vec, const Eigen::Isometry3d & coord)
{
  if (vec.size() % 4 != 0) {
  ...
  for (int i = 0; i < num_ranges; i++) {
    ...
    if (
      x1 <= coord.translation().x() && coord.translation().x() <= x2 &&
