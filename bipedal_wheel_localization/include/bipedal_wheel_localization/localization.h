...
// ros
#include <bipedal_wheel_msgs/LocationInfo.h>
#include <bipedal_wheel_msgs/Relocate.h>
...
public:
  ...
  bool RelocalizeCallback(
    bipedal_wheel_msgs::Relocate::Request & req, bipedal_wheel_msgs::Relocate::Response & res);
  geometry_msgs::PoseWithCovarianceStamped Isometry3d_to_PoseWithCovarianceStamped(
    const Eigen::Isometry3d & iso);
  void rotateScan(sensor_msgs::LaserScan::Ptr & scan, double angle);
  void rotatePointCloud(
    PointCloudT::Ptr & cloud_msg, const Eigen::Affine3f & rotation,
    const Eigen::Affine3f & robo_pose);
...
private:
  ...
  double
    ObstacleRemoval_Distance_Max;  //如果雷达点云中点在地图点云最近点大于此值，就认为该点为障碍点
  ...
  void PointCloudObstacleRemoval(
