...
{
FromTopicStateEstimate::FromTopicStateEstimate() : StateEstimateBase()
{
  ros::NodeHandle nh;
...
Eigen::Matrix<double, Eigen::Dynamic, 1> FromTopicStateEstimate::update(const ros::Time& /*time*/,
                                                                        const ros::Duration& /*period*/)
{
  nav_msgs::Odometry odom = *buffer_.readFromRT();

  updateAngular(quatToZyx(Eigen::Quaternion<double>(odom.pose.pose.orientation.w, odom.pose.pose.orientation.x,
                                                    odom.pose.pose.orientation.y, odom.pose.pose.orientation.z)),
                Eigen::Matrix<double, 3, 1>(odom.twist.twist.angular.x, odom.twist.twist.angular.y,
