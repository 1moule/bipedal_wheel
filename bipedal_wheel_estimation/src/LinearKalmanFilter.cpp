...
Eigen::Matrix<double, Eigen::Dynamic, 1> KalmanFilterEstimate::update(const ros::Time &time,
  ...
  z << baseVelAbsolute, linearAccelLocal_.x() + 0.02;
...
nav_msgs::Odometry KalmanFilterEstimate::getOdomMsg(const ros::Duration &period) {
  ...
  Eigen::Vector3d baseVelInOdom = quat_.inverse() * baseVelInBase;
  odom.twist.twist.linear.x = baseVelInBase.x();
  odom.twist.twist.linear.y = baseVelInBase.y();
  odom.twist.twist.linear.z = baseVelInBase.z();
